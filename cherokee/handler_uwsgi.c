		 ...
		 cherokee_buffer_t           *buffer)
{
	...
	size_t                    read  = 0;
	cherokee_handler_uwsgi_t *uwsgi = HDL_UWSGI(cgi_base);
	cherokee_request_t       *req   = HANDLER_REQ(cgi_base);

	ret = cherokee_socket_bufread (&uwsgi->socket, buffer, 4096, &read);

	switch (ret) {
	case ret_eagain:
		req->polling_aim.fd   = uwsgi->socket.socket;
...
build_header (cherokee_handler_uwsgi_t *hdl)
{
	...
	if (props->pass_request_body == true && props->pass_wsgi_vars == true) {
        	len = snprintf (tmp, sizeof(tmp), FMT_OFFSET, (CST_OFFSET)req->post.len);
...
cherokee_handler_uwsgi_read_post (cherokee_handler_uwsgi_t *hdl)
{
	...
	/* Send it
	 */
	ret = cherokee_post_send_to_socket (&req->post, &req->socket,
	...
	case ret_eagain:
		/* cherokee_post_send_to_socket() filled out req->polling_aim
		 */
		return ret_eagain;
