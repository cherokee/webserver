# -*- coding: utf-8 -*-
#
# Cherokee-admin's Liferay Wizard
#
# Authors:
...
#
# Tested:
# 2009/10/xx: Liferay CE / Cherokee 0.99.25
# 2010/04/15: Liferay CE / Cherokee 0.99.41
#
...
NOTE_WELCOME_H1 = N_("Welcome to the Liferay Wizard")
NOTE_WELCOME_P1 = N_('<a target="_blank" href="http://www.liferay.com/">Liferay Portal</a> is an enterprise web platform for building business solutions that deliver immediate results and long-term value.')
...
NOTE_COMMON_H1  = N_("Liferay Project")
NOTE_COMMON_SRC = N_('Hostname or IP of the server running Liferay. You can add more later to have the load balanced.')
...
PREFIX          = 'tmp!wizard!liferay'
URL_APPLY       = r'/wizard/vserver/liferay/apply'
...
class WebDirectory:
    def __call__ (self):
        ...
        table.Add (_('Web Directory'), CTK.TextCfg ('%s!new_webdir'%(PREFIX), False, {'value': "/liferay", 'class': 'noauto'}), _(NOTE_WEBDIR))
...
class Host:
    def __call__ (self):
        table = CTK.PropsTable()
        table.Add (_('New Host Name'),    CTK.TextCfg ('%s!new_host'%(PREFIX), False, {'value': 'liferay.example.com', 'class': 'noauto'}), _(NOTE_HOST))
...
class Welcome:
    def __call__ (self):
        ...
        cont += Wizard.Icon ('liferay', {'class': 'wizard-descr'})
        box = CTK.Box ({'class': 'wizard-welcome'})
        box += CTK.RawHTML ('<p>%s</p>' %(_(NOTE_WELCOME_P1)))
        box += CTK.RawHTML ('<p>%s</p>' %(_(NOTE_WELCOME_P2)))
        box += Wizard.CookBookBox ('cookbook_liferay')
        cont += box
...
VALS = [
    ...
    ('%s!bew_webdir'   %(PREFIX), validations.is_not_empty),
...
# VServer
CTK.publish ('^/wizard/vserver/liferay$',   Welcome)
CTK.publish ('^/wizard/vserver/liferay/2$', Common)
CTK.publish ('^/wizard/vserver/liferay/3$', Host)

# Rule
CTK.publish ('^/wizard/vserver/(\d+)/liferay$',   Welcome)
CTK.publish ('^/wizard/vserver/(\d+)/liferay/2$', Common)
CTK.publish ('^/wizard/vserver/(\d+)/liferay/3$', WebDirectory)
